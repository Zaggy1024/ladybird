...
ErrorOr<NonnullRefPtr<Bitmap>> YUVData::to_bitmap() const
{
    ...
    if (impl.bit_depth <= 8) {
        ...
        // Strides are in u16 element count for libyuv 10/12-bit functions (pointer arithmetic on uint16_t*)
        int result;
        if (impl.bit_depth <= 10) {
            if (impl.subsampling.x() && impl.subsampling.y()) {
                result = libyuv::I010ToARGBMatrix(shifted_y.data(), y_stride_16, shifted_u.data(), uv_stride_16,
                    shifted_v.data(), uv_stride_16, reinterpret_cast<u8*>(dst), dst_stride, yuv_constants, width, height);
            } else if (impl.subsampling.x() && !impl.subsampling.y()) {
                result = libyuv::I210ToARGBMatrix(shifted_y.data(), y_stride_16, shifted_u.data(), uv_stride_16,
                    shifted_v.data(), uv_stride_16, reinterpret_cast<u8*>(dst), dst_stride, yuv_constants, width, height);
            } else {
                result = libyuv::I410ToARGBMatrix(shifted_y.data(), y_stride_16, shifted_u.data(), uv_stride_16,
                    shifted_v.data(), uv_stride_16, reinterpret_cast<u8*>(dst), dst_stride, yuv_constants, width, height);
            }
        } else {
            // 12-bit
            if (impl.subsampling.x() && impl.subsampling.y()) {
                result = libyuv::I012ToARGBMatrix(shifted_y.data(), y_stride_16, shifted_u.data(), uv_stride_16,
                    shifted_v.data(), uv_stride_16, reinterpret_cast<u8*>(dst), dst_stride, yuv_constants, width, height);
            } else if (impl.subsampling.x() && !impl.subsampling.y()) {
    ...
    auto elapsed = MonotonicTime::now() - start;
    dbgln("YUV->RGB CPU conversion: {}x{} {}bit {}: {}μs",
