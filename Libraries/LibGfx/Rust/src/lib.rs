...
impl From<YUVMatrix> for YuvStandardMatrix {
    fn from(matrix: YUVMatrix) -> Self {
        match matrix {
            YUVMatrix::Identity => YuvStandardMatrix::Bt601,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn yuv_to_bgra_8bit(
    y_plane: *const u8,
    y_stride: u32,
    u_plane: *const u8,
    u_stride: u32,
    ...
    let result = match (subsampling_x, subsampling_y) {
        ...
        _ => yuv::yuv444_to_rgba(&planar_image, dst_slice, dst_stride, range.into(), matrix.into()),
    };
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn yuv_to_bgra_high_bit_depth(
    y_plane: *const u16,
    y_stride: u32,
    u_plane: *const u16,
    u_stride: u32,
    ...
    // Strides from C++ are in bytes; the yuv crate expects element counts for u16 planes.
    let y_stride_elems = y_stride / 2;
    let u_stride_elems = u_stride / 2;
    let v_stride_elems = v_stride / 2;

    let y_len = y_stride_elems as usize * height as usize;
    let uv_height = if subsampling_y {
        (height + 1) / 2
    } else {
        height
    } as usize;
    let uv_len = u_stride_elems as usize * uv_height;

    let planar_image = YuvPlanarImage {
        y_plane: unsafe { core::slice::from_raw_parts(y_plane, y_len) },
        y_stride: y_stride_elems,
        u_plane: unsafe { core::slice::from_raw_parts(u_plane, uv_len) },
        u_stride: u_stride_elems,
        v_plane: unsafe { core::slice::from_raw_parts(v_plane, uv_len) },
        v_stride: v_stride_elems,
        width,
        height,
    };
    ...
    let result = if bit_depth <= 10 {
        match (subsampling_x, subsampling_y) {
            ...
            _ => yuv::i410_to_rgba(&planar_image, dst_slice, dst_stride, range.into(), matrix.into()),
        }
    } else {
        ...
        match (subsampling_x, subsampling_y) {
            ...
            _ => yuv::i212_to_rgba(&planar_image, dst_slice, dst_stride, range.into(), matrix.into()),
        }
    };
