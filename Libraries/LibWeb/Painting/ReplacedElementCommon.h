...
inline Gfx::IntRect get_replaced_box_painting_area(PaintableBox const& paintable, DisplayListRecordingContext const& context, CSS::ObjectFit object_fit, Gfx::IntSize content_size)
{
    ...
    auto bitmap_aspect_ratio = [&] {
        return CSSPixels(content_size.height()) / content_size.width();
    };
    auto image_aspect_ratio = [&] {
        if (paintable_rect.width() == 0)
            return CSSPixelFraction(0, 1);
        return paintable_rect.height() / paintable_rect.width();
    };
