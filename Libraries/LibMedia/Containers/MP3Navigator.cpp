...
static EnclosingCachedRanges find_enclosing_cached_ranges_for_timestamp(Vector<MP3Navigator::CachedRange> const& cached_ranges, AK::Duration timestamp)
{
    EnclosingCachedRanges ranges { cached_ranges.first(), {} };
...
static Optional<size_t> scan_cached_range_if_changed(MediaStreamCursor& cursor, MP3Navigator::CachedRange& range, MediaStream::ByteRange const& byte_range)
{
    if (range.last_scanned_byte > byte_range.end) {
        ...
        return range.byte_start;
    }

    if (range.last_scanned_byte == byte_range.end)
        return {};

    auto old_last_scanned_byte = range.last_scanned_byte;
    auto old_duration_in_ticks = range.duration_in_ticks;
    scan_available_frames_for_cached_range(cursor, range, byte_range);
    if (range.last_scanned_byte == old_last_scanned_byte && range.duration_in_ticks == old_duration_in_ticks)
        return {};
    return range.byte_start;
}

static Optional<size_t> insert_cached_range_for_byte_range(MediaStreamCursor& cursor, Vector<MP3Navigator::CachedRange>& ranges, MediaStream::ByteRange const& byte_range, size_t first_frame_position, Optional<u64> file_size, AK::Duration total_duration)
{
    ...
    auto changed_byte = range.byte_start;
    insert_cached_range_sorted(ranges, range);
    return changed_byte;
}
...
DecoderErrorOr<SeekResult> MP3Navigator::seek_to_timestamp(AK::Duration timestamp) const
{
    ...
    FrameScanResult scan_result;
    {
        ...
        enclosing_cached_ranges = find_enclosing_cached_ranges_for_timestamp(m_cached_ranges, timestamp);
...
void MP3Navigator::update_cached_ranges(Vector<MediaStream::ByteRange> const& byte_ranges, MediaStreamCursor& cursor) const
{
    ...
    auto update_earliest_range_byte = [](Optional<size_t>& earliest_byte, size_t byte) {
        if (!earliest_byte.has_value() || byte < *earliest_byte)
            earliest_byte = byte;
    };

    Optional<size_t> earliest_changed_range_byte;
    auto file_size = m_stream->expected_size();
    ...
    while (byte_index < byte_ranges.size()) {
        ...
        if (cached_index < m_cached_ranges.size() && m_cached_ranges[cached_index].byte_start < byte_range.start) {
            update_earliest_range_byte(earliest_changed_range_byte, m_cached_ranges[cached_index].byte_start);
        ...
        if (cached_index >= m_cached_ranges.size() || m_cached_ranges[cached_index].byte_start >= byte_range.end) {
            auto changed_byte = insert_cached_range_for_byte_range(cursor, m_cached_ranges, byte_range, m_first_frame_position, file_size, m_total_duration);
            if (changed_byte.has_value()) {
                update_earliest_range_byte(earliest_changed_range_byte, *changed_byte);
                cached_index++;
            }
            byte_index++;
            continue;
        ...
        auto changed_byte = scan_cached_range_if_changed(cursor, cached_range, byte_range);
        if (changed_byte.has_value())
            update_earliest_range_byte(earliest_changed_range_byte, *changed_byte);

        auto kept_range_byte_start = cached_range.byte_start;
        while (cached_index + 1 < m_cached_ranges.size() && m_cached_ranges[cached_index + 1].byte_start < byte_range.end) {
            update_earliest_range_byte(earliest_changed_range_byte, kept_range_byte_start);
    ...
    if (earliest_changed_range_byte.has_value()) {
        auto first_range_to_reproject = m_cached_ranges.find_if([&](auto const& range) {
            return range.byte_start > *earliest_changed_range_byte;
        });
        if (!first_range_to_reproject.is_end())
            reproject_cached_range_times_from(first_range_to_reproject.index());
    }
}

void MP3Navigator::reproject_cached_range_times_from(size_t start_index) const
{
    auto index = max<size_t>(start_index, 1);
    if (index >= m_cached_ranges.size())
        return;

    auto file_size = m_stream->expected_size();
    for (size_t i = index; i < m_cached_ranges.size(); i++) {
