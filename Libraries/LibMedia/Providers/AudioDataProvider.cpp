...
DecoderErrorOr<NonnullRefPtr<AudioDataProvider>> AudioDataProvider::try_create(NonnullRefPtr<MutexedDemuxer> const& demuxer, Track const& track)
{
    ...
    DECODER_TRY_ALLOC(converter->set_output_sample_specification(Audio::SampleSpecification(44100, Audio::ChannelMap::stereo())));

    auto thread_data = DECODER_TRY_ALLOC(try_make_ref_counted<AudioDataProvider::ThreadData>(demuxer, track, move(decoder), move(converter)));
...
AudioDataProvider::ThreadData::ThreadData(NonnullRefPtr<MutexedDemuxer> const& demuxer, Track const& track, NonnullOwnPtr<AudioDecoder>&& decoder, NonnullOwnPtr<Audio::Converter>&& converter)
...
void AudioDataProvider::ThreadData::seek(AK::Duration timestamp, SeekCompletionHandler&& completion_handler)
{
    ...
    m_wait_condition.broadcast();
    dbgln("{}: seek to {}", m_track.identifier(), timestamp);
}
...
DecoderErrorOr<void> AudioDataProvider::ThreadData::retrieve_next_block(AudioBlock& block)
{
    ...
    dbgln("{}: got block @ {}", m_track.identifier(), block.timestamp());
    ...
    if (block.timestamp_in_samples() < m_last_sample){
        dbgln("  pushed block forward by {} samples", m_last_sample - block.timestamp_in_samples());
...
void AudioDataProvider::ThreadData::resolve_seek(u32 seek_id)
{
    process_seek_on_main_thread(seek_id, [self = NonnullRefPtr(*this)] {
        ...
        if (handler)
            handler();
        dbgln("{}: seek complete", self->m_track.identifier());
    });
}
