...
{
    return AK::Duration::from_time_units(m_next_frame_to_write.load(), 1, m_sample_specification.sample_rate());
}
...
PipelineStatus AudioMixer::pull(AudioBlock& into)
{
    ...
    Threading::MutexLocker locker { m_mutex };
    auto buffer_start_frame = m_next_frame_to_write.load();
