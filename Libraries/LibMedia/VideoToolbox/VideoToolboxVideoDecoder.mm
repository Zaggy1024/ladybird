...
DecoderErrorOr<void> VideoToolboxVideoDecoder::ensure_session_for_frame(CodedFrame const& coded_frame)
{
    ...
    VTDecompressionOutputCallbackRecord callback {
        [](void* decoder_reference, void*, OSStatus status, VTDecodeInfoFlags, CVImageBufferRef image_buffer, CMTime presentation_timestamp, CMTime presentation_duration) {
            auto& decoder = *static_cast<VideoToolboxVideoDecoder*>(decoder_reference);
            auto timestamp = duration_from_cm_time(presentation_timestamp);
            auto duration = duration_from_cm_time(presentation_duration);
            decoder.note_decode_completed(status, image_buffer, timestamp, duration);
        },
        this
    };
...
void VideoToolboxVideoDecoder::note_decode_completed(i32 status, void* image_buffer, AK::Duration timestamp, AK::Duration duration)
{
    Sync::MutexLocker locker { m_output_mutex };
    ...
    if (status != noErr)
        ...
        enqueue_decoded_output_while_locked(image_buffer, timestamp, duration);
...
void VideoToolboxVideoDecoder::enqueue_decoded_output_while_locked(void* image_buffer, AK::Duration timestamp, AK::Duration duration)
{
    ...
    DecodedOutput output {
        ...
        .cicp = m_session != nullptr ? m_session->cicp : CodingIndependentCodePoints {},
    };
...
DecoderErrorOr<void> VideoToolboxVideoDecoder::receive_coded_data(CodedFrame const& coded_frame, DecodeIntent intent)
{
    ...
    if (CMBlockBufferCreateWithMemoryBlock(kCFAllocatorDefault, nullptr, data.size(), kCFAllocatorDefault, nullptr, 0, data.size(), 0, block_buffer.out()) != noErr)
        return DecoderError::with_description(DecoderErrorCategory::Memory, "Failed to allocate a VideoToolbox block buffer"sv);
    if (CMBlockBufferReplaceDataBytes(data.data(), block_buffer.ref(), 0, data.size()) != noErr)
        return DecoderError::with_description(DecoderErrorCategory::Memory, "Failed to fill a VideoToolbox block buffer"sv);
    ...
    auto status = VTDecompressionSessionDecodeFrame(m_session->session, sample_buffer.ref(), flags, nullptr, &info_flags);
    if (status != noErr) {
        Sync::MutexLocker locker { m_output_mutex };
        note_frame_left_the_media_engine_while_locked();
...
{
    if (m_session != nullptr) {
        VTDecompressionSessionFinishDelayedFrames(m_session->session);
