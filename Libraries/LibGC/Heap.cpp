...
NO_SANITIZE_ADDRESS void Heap::gather_conservative_roots(HashMap<Cell*, HeapRoot>& roots, HashTable<HeapBlock*> const& all_live_heap_blocks, Vector<StackFrameInfo>* out_stack_frames)
{
    ...
    if (out_stack_frames) {
        ...
        if (!frame_starts.is_empty()) {
            ...
            auto format_frame_label = [](cpptrace::stacktrace_frame const& sf) -> String {
                StringBuilder label;
                if (!sf.symbol.empty()) {
                    label.append(StringView(sf.symbol.c_str(), sf.symbol.length()));
                    if (sf.line.has_value()) {
                        auto filename = StringView { sf.filename.c_str(), sf.filename.length() };
                        auto last_slash = filename.find_last('/');
                        if (last_slash.has_value())
                            filename = filename.substring_view(*last_slash + 1);
                        label.appendff(" {}:{}", filename, sf.line.value());
                    }
                }
