...
public:
    ...
    // Enqueue, evicting the oldest element if the queue is full. Producer-only
    // operation; safe against concurrent consumers (eviction uses the same CAS-based
    // dequeue path). Returns the evicted element if eviction occurred.
    Optional<ValueType> enqueue_with_eviction(ValueType to_insert)
    {
        VERIFY(!m_queue.is_null());
        Optional<ValueType> evicted;
        auto first_attempt = enqueue(to_insert);
        if (!first_attempt.is_error())
            return evicted;
        VERIFY(first_attempt.error() == QueueStatus::Full);

        auto dequeued = dequeue();
        if (!dequeued.is_error())
            evicted = dequeued.release_value();
        // If a concurrent consumer drained the head before us, the slot is free either way.

        auto retry = enqueue(to_insert);
        VERIFY(!retry.is_error());
        return evicted;
    }
...
private:
    ...
    public:
        ...
        ~RefCountedSharedMemorySPCQ() = default;
